...
- **Output Directory:** `app/.next` (alineado con `vercel.json`)
...
2. Configurar `Output Directory` en `app/.next` (igual que `vercel.json`)
3. Redeploy
...
3. Usar el build de compatibilidad del repo (genera `app/.next` y enlaza `app/node_modules`)
