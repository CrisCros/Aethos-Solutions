...
2. Keep **Output Directory** aligned with `vercel.json` (`app/.next`).
3. Redeploy after saving settings.
4. Use the repository compatibility build (`npm run build:vercel`) which generates `app/.next` and links `app/node_modules` for legacy Vercel path resolution.
...
- Use the compatibility build command: `npm run build:vercel`
